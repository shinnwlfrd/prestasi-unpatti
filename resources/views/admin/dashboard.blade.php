...
@section('content')
    ...
    <style>
            ...
            @media print {
                /* Hide UI Elements */
                aside, header, nav, .no-print, button, form, .toast-container, [role="button"] {
        ...
        <div class="print-report-header">
            ...
            <p class="text-[10px] mt-2 italic text-gray-400">Dicetak pada: {{ now()->format('d/m/Y H:i:s') }} oleh {{ auth()->user()->name }}</p>
        </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form method="GET" action="{{ route('admin.dashboard') }}" class="flex items-center gap-2">
                        <select name="period" onchange="this.form.submit()"
                            ...
                            <option value="all" {{ request('period') === 'all' || (!request('period') && !isset($selectedPeriod)) ? 'selected' : '' }}>Semua
                                Periode</option>
                            @if(isset($periods))
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.export.achievements', ['format' => 'excel', 'period' => request('period', 'all')]) }}" 
                           class="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors shadow-sm text-sm font-medium">
                        ...
                        <a href="{{ route('admin.export.achievements', ['format' => 'csv', 'period' => request('period', 'all')]) }}" 
                           class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-sm text-sm font-medium">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($isInactivePeriod)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col">
                        ...
                        <div class="flex-1 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                                <div class="flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Banding dari mahasiswa yang perlu direview
                                        </p>
                                    </div>
                                    @if(isset($appealsPending) && $appealsPending->isNotEmpty())
                                        <span
                                            class="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-bold rounded-full">
                                ...
                                @else
                                    <div class="space-y-3 max-h-[400px] overflow-y-auto">
                                        @foreach($appealsPending as $appeal)
                                            <div
                                                class="flex items-center justify-between p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors">
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate">
                                                        {{ $appeal->studentAchievement?->event_name }}
                                                    </p>
                                                    <div class="flex items-center gap-2 mt-1">
                                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                                            {{ $appeal->studentAchievement?->student?->name }}
                                                        </p>
                                                        <span class="text-xs text-gray-400">•</span>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                                            {{ $appeal->studentAchievement?->student?->faculty_name }}
                                                        </p>
                                                    </div>
