...
<div class="space-y-6">
    <!-- Back Button -->
    <div>
        <a href="{{ route('admin.appeals.index') }}" class="inline-flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-sm text-red-700 dark:text-red-400">{{ $achievement->faculty_validated_at?->format('d M Y H:i') }}</span>
                            </div>
                            <p class="text-sm text-red-800 dark:text-red-300 mb-1">
                                Validator: <strong>{{ $achievement->facultyValidator?->name }}</strong>
                            </p>
                            <div class="mt-2 p-3 bg-white dark:bg-gray-800 rounded border border-red-200 dark:border-red-700">
                                <p class="text-sm text-gray-700 dark:text-gray-300">{{ $achievement->faculty_notes ?? 'Tidak ada catatan' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="p-3 bg-white dark:bg-gray-800 rounded border border-blue-200 dark:border-blue-700">
                                <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $appeal->reason }}</p>
                            </div>
                            @if($appeal->additional_notes)
                            <div class="mt-2 p-3 bg-white dark:bg-gray-800 rounded border border-blue-200 dark:border-blue-700">
                                <p class="text-xs text-blue-700 dark:text-blue-400 font-medium mb-1">Catatan Tambahan:</p>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <form action="{{ route('admin.appeals.review', $appeal) }}" method="POST" class="space-y-4">
                    ...
                    <!-- Decision Info -->
                    <div id="approve-info" class="hidden p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                        <p class="text-sm text-green-800 dark:text-green-300">
                            <strong>Terima Banding:</strong> Prestasi akan dikembalikan ke status "Submitted" untuk direview ulang oleh validator fakultas.
                        </p>
                    </div>

                    <div id="reject-info" class="hidden p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                        <p class="text-sm text-red-800 dark:text-red-300">
                            <strong>Tolak Banding:</strong> Prestasi tetap dalam status "Revisi Fakultas". Mahasiswa harus memperbaiki sesuai catatan validator.
                        </p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="p-4 rounded-lg {{ $appeal->status === 'approved' ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' }}">
                    <div class="flex items-start gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 {{ $appeal->status === 'approved' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }}">
                            <svg class="w-5 h-5 {{ $appeal->status === 'approved' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex-1">
                            <p class="font-medium {{ $appeal->status === 'approved' ? 'text-green-900 dark:text-green-300' : 'text-red-900 dark:text-red-300' }} mb-2">
                                Banding {{ $appeal->status === 'approved' ? 'Diterima' : 'Ditolak' }}
                            </p>
                            <p class="text-sm {{ $appeal->status === 'approved' ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300' }} mb-1">
                                Direview oleh: <strong>{{ $appeal->reviewer?->name }}</strong>
                            </p>
                            <p class="text-sm {{ $appeal->status === 'approved' ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400' }}">
                                {{ $appeal->reviewed_at?->format('d M Y H:i') }}
                            </p>
                            @if($appeal->admin_notes)
                            <div class="mt-2 p-3 bg-white dark:bg-gray-800 rounded border {{ $appeal->status === 'approved' ? 'border-green-200 dark:border-green-700' : 'border-red-200 dark:border-red-700' }}">
