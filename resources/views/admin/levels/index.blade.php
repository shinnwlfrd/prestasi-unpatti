...
@section('content')
<div class="space-y-6" x-data="{ showModal: false, editMode: false, levelId: null, levelName: '', levelDescription: '', levelPoints: '' }">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Level Prestasi</h2>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Kelola level prestasi mahasiswa</p>
        </div>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <form action="{{ route('admin.levels.destroy', $level) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus level ini?')">
                                @csrf
                                @method('DELETE')
    ...
    <div x-show="showModal" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal"
                ...
                <form :action="editMode ? '{{ route('admin.levels.index') }}/' + levelId : '{{ route('admin.levels.store') }}'" method="POST">
                    @csrf
                    ...
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white" x-text="editMode ? 'Edit Level' : 'Tambah Level'"></h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Poin yang didapat mahasiswa untuk prestasi level ini</p>
                            </div>
                        </div>
                    </div>

                    <div class="bg-gray-50 dark:bg-gray-900/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                        <button type="submit" class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-purple-600 text-base font-medium text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:ml-3 sm:w-auto sm:text-sm">
                            <span x-text="editMode ? 'Update' : 'Simpan'"></span>
                        </button>
                        <button type="button" @click="showModal = false" class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:mt-0 sm:w-auto sm:text-sm">
                            Batal
                        </button>
                    </div>
                </form>
