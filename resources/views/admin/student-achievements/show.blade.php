                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $document->type_name ?? $document->document_type }}</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ $document->file_name ?? basename($document->file_path ?? '') }}</p>
                                    </div>
                                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl lg:rounded-xl border border-gray-200 dark:border-gray-700 p-6 lg:p-8 xl:p-10 desktop-card-hover">
                    ...
                    <div class="space-y-3">
                        ...
                        @if($achievement->is_resubmission && $achievement->resubmission_count > 0)
                            <div
                                class="w-full px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                ...
                                @if($achievement->resubmission_reason)
                                    <p class="text-sm text-blue-600 dark:text-blue-300 mt-2">{{ $achievement->resubmission_reason }}
                                    </p>
                                @endif
                                @if($achievement->last_resubmitted_at)
                                    <p class="text-xs text-blue-500 dark:text-blue-400 mt-1">
                                        {{ $achievement->last_resubmitted_at->format('d M Y H:i') }}</p>
                                @endif
                            </div>
                        @endif
                    ...
                    <div class="space-y-4">
                        ...
                        @if($achievement->submitted_at)
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1 pb-4">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $achievement->submitted_at->format('d M Y H:i') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($achievement->faculty_validated_at)
                            <div class="flex gap-3">
                                <div class="flex flex-col items-center">
                                    <div
                                        class="w-8 h-8 {{ in_array($achievement->validation_status, ['faculty_approved', 'university_review', 'university_approved']) ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }} rounded-full flex items-center justify-center">
                                        <svg class="w-4 h-4 {{ in_array($achievement->validation_status, ['faculty_approved', 'university_review', 'university_approved']) ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}"
                                ...
                                <div class="flex-1 pb-4">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ in_array($achievement->validation_status, ['faculty_approved', 'university_review', 'university_approved']) ? 'Disetujui' : 'Ditolak' }}
                                        Fakultas
                                    </p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $achievement->faculty_validated_at->format('d M Y H:i') }}</p>
                        ...
                        @if($achievement->university_validated_at)
                            <div class="flex gap-3">
                                <div class="flex flex-col items-center">
                                    <div
                                        class="w-8 h-8 {{ $achievement->validation_status == 'university_approved' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }} rounded-full flex items-center justify-center">
                                        <svg class="w-4 h-4 {{ $achievement->validation_status == 'university_approved' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}"
                                            fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ $achievement->validation_status == 'university_approved' ? 'Disetujui' : 'Ditolak' }}
                                        Universitas
                                    </p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $achievement->university_validated_at->format('d M Y H:i') }}</p>
                                    @if($achievement->universityValidator)
                                        <p class="text-xs text-gray-500 dark:text-gray-400">oleh
                                            {{ $achievement->universityValidator->name }}</p>
                                    @endif
                                    @if($achievement->university_notes)
                                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-1 italic">
                                            "{{ $achievement->university_notes }}"</p>
                                    @endif
                                </div>
                            </div>
