            ...
            @forelse($categories as $category)
                <div
                    ...
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span
                            class="px-2 py-1 text-xs font-semibold rounded-full {{ $category->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-900/30 dark:text-gray-400' }}">
                    ...
                    <div class="flex items-center gap-2 mt-4">
                        ...
                        <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="flex-1"
                            @submit.prevent="window.showConfirm('Yakin ingin menghapus kategori ini?', () => $el.submit())">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="w-full px-3 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors text-sm font-medium">
                                Hapus
                            </button>
                        </form>
                    </div>
