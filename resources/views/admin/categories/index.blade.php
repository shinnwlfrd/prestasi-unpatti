            ...
            @forelse($categories as $category)
                <div
                    ...
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center flex-shrink-0">
                            ...
                            <div>
                                ...
                                @if($category->is_active)
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-full text-xs font-medium mt-1">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 text-gray-700 dark:bg-gray-900/30 dark:text-gray-400 rounded-full text-xs font-medium mt-1">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            @click="showModal = true; editMode = true; categoryId = {{ $category->id }}; categoryName = '{{ addslashes($category->name) }}'; categoryDescription = '{{ addslashes($category->description ?? '') }}'"
                        ...
                        <form action="{{ route('admin.categories.destroy', $category) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="w-full px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @empty
                <div class="col-span-full">
                    <div
                        ...
                        <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">Klik tombol "Tambah Kategori" untuk membuat
                            kategori baru</p>
                    </div>
                </div>
            @endforelse
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal" x-transition:enter="ease-out duration-300"
                    ...
                    <form
                        :action="editMode ? '{{ route('admin.categories.index') }}/' + categoryId : '{{ route('admin.categories.store') }}'"
                        method="POST">
                        @csrf
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                            <button type="submit"
                                class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-purple-600 text-base font-medium text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
