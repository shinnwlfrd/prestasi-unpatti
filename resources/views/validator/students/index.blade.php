...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <form method="GET" action="{{ route($routePrefix . '.students.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Search -->
            <div>
            ...
            <div>
                ...
                <input type="text" name="program_study" value="{{ request('program_study') }}" 
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($students as $student)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($student->name) }}&background=10b981&color=fff" alt="">
                                    </div>
                                    <div class="ml-4">
                            ...
                            @if($showFaculty)
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-white">{{ Str::limit($student->faculty ?? '-', 30) }}</div>
                            </td>
                            @endif
                            @if($showDepartment)
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-white">{{ Str::limit($student->department ?? '-', 30) }}</div>
                            </td>
                            @endif
                            @if($showProgramStudy)
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-white">{{ Str::limit($student->program_study ?? '-', 30) }}</div>
                            </td>
                            @endif
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($student->gpa)
                                    <div class="flex items-center">
                                        <span class="text-sm font-semibold {{ $student->gpa >= 3.5 ? 'text-green-600 dark:text-green-400' : ($student->gpa >= 3.0 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400') }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 4 + ($showFaculty ? 1 : 0) + ($showDepartment ? 1 : 0) + ($showProgramStudy ? 1 : 0) + ($showGPA ? 1 : 0) }}" class="px-6 py-12 text-center">
