    ...
    <div class="max-w-3xl mx-auto space-y-5 sm:space-y-6 animate-fade-in px-4 sm:px-6 pb-20">
        ...
        @if(!$isStudent && auth()->check() && auth()->user()->isSuperAdmin())
            @php
                ...
                $isValidatorPage = request()->routeIs('validator.*');
                $isPimpinanPage = request()->routeIs('pimpinan.*');
                
                // Build available quick access links (exclude current page)
                $quickAccessLinks = [];
                ...
                if (!$isAdminPage) {
                    $quickAccessLinks[] = [
                        ...
                        'title' => 'Halaman Admin',
                        'description' => 'Akses dashboard admin',
                        'color' => 'blue',
                ...
                if (!$isPimpinanPage) {
                    $quickAccessLinks[] = [
                        ...
                        'description' => 'Akses dashboard pimpinan (Level Universitas)',
                        'color' => 'purple',
            ...
            @if(count($quickAccessLinks) > 0)
                <x-card title="Akses Cepat">
                    <div class="grid grid-cols-1 sm:grid-cols-{{ count($quickAccessLinks) > 1 ? '2' : '1' }} gap-4">
                        @foreach($quickAccessLinks as $link)
                            <a href="{{ $link['url'] }}"
                                class="group p-4 rounded-xl bg-gradient-to-br from-{{ $link['color'] }}-50 to-{{ $link['color'] }}-100 dark:from-{{ $link['color'] }}-900/20 dark:to-{{ $link['color'] }}-900/30 border-2 border-{{ $link['color'] }}-200 dark:border-{{ $link['color'] }}-800 hover:border-{{ $link['color'] }}-400 dark:hover:border-{{ $link['color'] }}-600 transition-all duration-200 hover:shadow-lg">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-xl bg-{{ $link['color'] }}-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $link['icon'] }}" />
                                        </svg>
                                    </div>
                                    <div class="flex-1">
                                        <h3 class="font-semibold text-gray-900 dark:text-white group-hover:text-{{ $link['color'] }}-600 dark:group-hover:text-{{ $link['color'] }}-400 transition-colors">
                                            {{ $link['title'] }}
                                        </h3>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">
                                            {{ $link['description'] }}
                                    ...
                                    <svg class="w-5 h-5 text-{{ $link['color'] }}-500 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
