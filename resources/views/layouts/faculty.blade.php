...
<head>
    ...
    <title>@yield('title', 'Dashboard') - Sistem Prestasi Mahasiswa</title>
...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    @php
        ...
        $gradientFrom = $isOperator ? 'from-emerald-500' : 'from-purple-500';
        $gradientTo = $isOperator ? 'to-teal-600' : 'to-indigo-600';
        $bgLight = $isOperator ? 'bg-emerald-100' : 'bg-purple-100';
        $bgDark = $isOperator ? 'dark:bg-emerald-900/30' : 'dark:bg-purple-900/30';
        $textLight = $isOperator ? 'text-emerald-600' : 'text-purple-600';
        $textDark = $isOperator ? 'dark:text-emerald-400' : 'dark:text-purple-400';
        $hoverBg = $isOperator ? 'hover:bg-emerald-100' : 'hover:bg-purple-100';
        $hoverBgDark = $isOperator ? 'dark:hover:bg-emerald-900/30' : 'dark:hover:bg-purple-900/30';

        // Determine routes prefix
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            class="fixed h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 w-64 transition-transform duration-300 z-50">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 bg-gradient-to-br {{ $gradientFrom }} {{ $gradientTo }} rounded-xl flex items-center justify-center flex-shrink-0">
            ...
            <!-- Read-Only Notice (Pimpinan only) -->
            @if($isPimpinan)
            ...
            <nav
                class="p-4 space-y-2 overflow-y-auto {{ $isPimpinan ? 'h-[calc(100vh-145px)]' : 'h-[calc(100vh-73px)]' }}">
                <!-- Dashboard -->
                <a href="{{ route($routePrefix . '.dashboard') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs($routePrefix . '.dashboard') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                ...
                <div class="pt-2">
                    <p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                        Data {{ $isPimpinan ? '(Read-Only)' : '' }}
                    </p>
                </div>

                <!-- Mahasiswa -->
                <a href="{{ route($routePrefix . '.students.index') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs($routePrefix . '.students.*') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if($isPimpinan)
                        <span
                            class="ml-auto px-1.5 py-0.5 text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded">View</span>
                    @endif
                </a>

                <!-- Validasi Section (Operator only) -->
                @if($isOperator)
                    <div class="pt-2">
                    ...
                    <a href="{{ route('validator.pending.index') }}"
                        class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('validator.pending.*') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route($routePrefix . '.history') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs($routePrefix . '.history') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route($routePrefix . '.sk.index') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs($routePrefix . '.sk.*') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if($isOperator)
                    ...
                    <a href="{{ route('validator.submit.form') }}"
                        class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('validator.submit.*') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('profile') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('profile') ? $bgLight . ' ' . $bgDark . ' ' . $textLight . ' ' . $textDark : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <header
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    @php
                        ...
                        $currentRoleObj = $currentRoleId ? $activeRoles->firstWhere('id', $currentRoleId) : $activeRoles->first();
                    @endphp
                    @if($activeRoles->count() > 1)
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" type="button"
                                class="flex items-center gap-2 px-3 py-2 text-sm {{ $bgLight }} {{ $bgDark }} {{ $textLight }} {{ $textDark }} rounded-lg {{ $hoverBg }} {{ $hoverBgDark }} transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                                <span
                                    class="hidden sm:inline">{{ $currentRoleObj ? $currentRoleObj->getRoleDisplayName() : 'Role' }}</span>
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7" />
                                </svg>
                                        ...
                                        <form method="POST" action="{{ route('role.switch') }}" class="block">
                                            ...
                                            <button type="submit"
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    <div
                                                        class="w-8 h-8 bg-gradient-to-br {{ $gradientFrom }} {{ $gradientTo }} rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    <form action="{{ $user->last_login_method === 'sso' ? route('sso.logout') : route('logout') }}"
                        method="POST">
                        @csrf
                        <button type="submit"
                            ...
                            title="Logout{{ $user->last_login_method === 'sso' ? ' (SSO)' : '' }}">
