    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <dl class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $document->studentAchievement?->event_name }}</dd>
            </div>
            <div>
                <dt class="text-sm text-gray-500 dark:text-gray-400">Diupload</dt>
                <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $document->created_at->format('d M Y H:i') }}</dd>
            </div>
            @if($document->verified_at)
            <div>
                <dt class="text-sm text-gray-500 dark:text-gray-400">Diverifikasi oleh</dt>
                <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $document->verifier?->name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm text-gray-500 dark:text-gray-400">Tanggal Verifikasi</dt>
                <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $document->verified_at->format('d M Y H:i') }}</dd>
            </div>
            @endif
        </dl>
                    ...
                    <div class="relative flex gap-4">
                        ...
                        <div class="flex-1 bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
                            <div class="flex items-start justify-between">
                                <div>
                                    <x-badge :type="$revision->action_badge" size="sm">{{ $revision->action_label }}</x-badge>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        oleh <span class="font-medium text-gray-900 dark:text-white">{{ $revision->performer?->name ?? 'System' }}</span>
                                    </p>
                                </div>
